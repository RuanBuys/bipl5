test_that("mdsDisplay naming helpers use stable conventions", {
  expect_identical(bipl5:::mdsDisplay_name(c(1, 2)), "mdsDisplay_12")
...
test_that("mdsDisplay constructors attach expected classes and fields", {
  ...
  mdsDisplay_obj <- bipl5:::new_bipl5_mdsDisplay(
    list(mdsDisplay = list(trace_data = list()), fit_qual = "PC 1 & 2"),
    data = data_obj
  )
  expect_s3_class(mdsDisplay_obj, "bipl5_mdsDisplay")
  expect_identical(mdsDisplay_obj$Data, data_obj)
  ...
  top <- bipl5:::new_bipl5_biplot(
    mdsDisplays = list(mdsDisplay_12 = mdsDisplay_obj),
    fit_measures = fit_obj,
    meta = list(pc_info = list(mdsDisplay_12 = list(label = "PC 1 & 2"))),
    biplot_type = "pca"
  )
  expect_s3_class(top, "bipl5_biplot")
  expect_s3_class(top, "pca")
  expect_identical(top$mdsDisplay_12, mdsDisplay_obj)
...
test_that("dim_label and deparse_path handle supported inputs", {
  ...
  expect_identical(bipl5:::deparse_path(quote(mdsDisplay_12)), "mdsDisplay_12")
  expect_identical(
    bipl5:::deparse_path(quote(mdsDisplay_12$Data$sample_coordinates)),
    c("mdsDisplay_12", "Data", "sample_coordinates")
  )
  expect_error(
    bipl5:::deparse_path(quote(mdsDisplay_12 + Data)),
    "extract\\(\\) expects a path"
  )
})

test_that("mdsDisplay constructors and fit subtree printers emit readable tree output", {
  ...
  expect_match(bp_out, "mdsDisplay_12 \\[PC 1 & 2\\]")
  expect_match(bp_out, "fit_measures")

  mdsDisplay_out <- paste(capture.output(print(bp$mdsDisplay_12)), collapse = "\n")
  expect_match(mdsDisplay_out, "bipl5_mdsDisplay")
  expect_match(mdsDisplay_out, "sample_coordinates")

  data_out <- paste(capture.output(print(bp$mdsDisplay_12$Data)), collapse = "\n")
  ...
  subtree_out <- paste(
    capture.output(bipl5:::print_data_subtree(bp$mdsDisplay_12$Data, prefix = "")),
    collapse = "\n"
  )
  ...
  cva_subset <- bipl5:::subset_biplot(wrapped_cva(), "mdsDisplay_12")
